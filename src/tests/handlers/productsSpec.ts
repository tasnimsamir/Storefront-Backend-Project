...
//       const response = await request.get('/products').set('Authorization', `Bearer ${(await TestUser).testToken}`);
//       expect(response.status).toBe(200);
//     });


//     it('Endpoint: /products/:product_id [GET]', async (): Promise<void> => {
//       const response = await request.get(`/products/${(await TestProduct).testProduct.id}`).set('Authorization', `Bearer ${(await TestUser).testToken}`);
//       expect(response.status).toBe(200);
//     });
//   });
